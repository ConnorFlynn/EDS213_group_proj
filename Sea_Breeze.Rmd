...
east_sst <- chloro_wind_sst %>% 
  filter(location == "east") %>% #filter for site
  select(date, mean_sst)

east_wind <- chloro_wind_sst %>% 
  filter(location == "east") %>% 
  select(date, mean_wind)
...
```{r}
# plot of wind
ggplot(data = chloro_wind_sst, aes(x = date, y = mean_wind, color = location)) +
  geom_line() +
  labs(x = "Date",
       y = "Daily Average Wind Speed (knots)",
       title = "Daily Average Wind Speeds from East, West, and Santa Monica Buoys")
```
