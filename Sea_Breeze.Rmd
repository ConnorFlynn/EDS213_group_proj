...
#Wind data for each buoy and data
tab_E <- read.table(here("data","east_wind.txt"), comment="", header=TRUE)
write.csv(tab_E, "east_wind.csv", row.names=F, quote=F)

E_wind <- read.csv(here("east_wind.csv")) %>% 
  add_column(location = "east") %>% 
...
tab_W <- read.table(here("data","west_wind.txt"), comment="", header=TRUE)
write.csv(tab_W, "west_wind.csv", row.names=F, quote=F)

W_wind <- read.csv(here("west_wind.csv"))%>% 
  add_column(location = "west") %>% 
  select(c("X.YY", "MM", "DD", "WSPD", "location"))  %>% 
  rename(year = X.YY,
         month = MM,
...
tab_SM <- read.table(here("data","SM_wind.txt"), comment="", header=TRUE)
write.csv(tab_SM, "SM_wind.csv", row.names=F, quote=F)

SM_wind <- read.csv(here("SM_wind.csv"))%>% 
  add_column(location = "SM") %>% 
  select(c("X.YY", "MM", "DD", "WSPD", "location"))  %>% 
  rename(year = X.YY,
         month = MM,
...
final_chloro_avg <- chloro_clean %>% 
  group_by(location, date) %>%
  summarize(mean_chloro = mean(chlorophyll))

```

```{r}

wind_sst <- left_join(wind_avg, final_sst_avg, by = c("date", "location"))
...
ggplot(data = west) +
  #geom_point(aes(x = date, y = mean_sst, fill = "SST"), col = "blue") +
  geom_point(aes(x = date, y = mean_wind, fill = "Wind"), col = "red")
